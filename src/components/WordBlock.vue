  ...
  >
    ...
    <template slot="popover">
      ...
      <div v-if="words.length === 0 && loading === false">
        ...
        |
        <a
          :href="`https://www.frenchdict.net/translate/${text}`"
          target="_blank"
          >FrenchDict</a
        >
      </div>
...
export default {
  ...
  methods: {
    ...
    async allForms() {
      ...
      if (this.words.length > 0) {
        for (let word of this.words) {
          let wordForms = (await (await this.$openFrench).wordForms(word)) || []
          wordForms = wordForms
    ...
    async lookup() {
      let words = await (await this.$openFrench).lookupFuzzy(this.text)
      if (words) {
        for (let word of words) {
          if (word && word.matches) {
            for (let match of word.matches) {
              match.form = await (await this.$openFrench).accent(match.form)
              match.field = await (await this.$openFrench).stylize(match.field)
              match.number = await (await this.$openFrench).stylize(
                match.number
              )
              match.table = await (await this.$openFrench).stylize(match.table)
            }
          }
        }
      }
