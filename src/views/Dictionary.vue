        ...
        <div class="row">
          <div class="col-sm-12">
            <div class="text-center">
              <h2 class="mb-4">For the love of French words.</h2>
...
export default {
  ...
  methods: {
    ...
    show(entry) {
      this.entry = entry
      document.title = `${entry.bare} (${entry.english}) | French Zero to Hero`
    },
    async route() {
      if (this.method && this.args) {
        if (this.method === 'freedict') {
          if (this.args === 'random') {
            ...
            this.entry = await (await this.$openFrench).get(this.args)
          }
        } else {
          if (!this.entry) {
            this.random()
    ...
    async random() {
      let randomId = (await (await this.$openFrench).random()).id
